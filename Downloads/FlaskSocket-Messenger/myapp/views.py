...
@views.route('/leave')
def leave():
    session.clear() # Clears the logged in user
    return redirect(url_for('views.login'))
    """
...
def remove_chat(room_id):
    """
    Removes the chat from the current user's sidebar list.
    """
    current_user_id = session["user"]["id"]
    current_chat = Chat.query.filter_by(user_id=current_user_id).first()

    if current_chat:
        # Filter out the chat with the matching room_id
        updated_list = [c for c in current_chat.chat_list if c['room_id'] != room_id]
        
        # Update the list and flag it as modified for SQLAlchemy
        current_chat.chat_list = updated_list
        flag_modified(current_chat, "chat_list")
...
def clear_chat(room_id):
    """
    Deletes all messages associated with the given room_id.
    """
    # Delete all ChatMessage records with this room_id
    try:
    ...
    # Redirect back to the chat view (preserving the room if it's open, 
    # though it will now be empty)
